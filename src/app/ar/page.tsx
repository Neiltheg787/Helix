...
import {
  getArCompatibilityReport,
  type ArCompatibilityReport,
} from "@/lib/ar-compatibility";
...
function ArInner() {
  ...
  const [compatibility, setCompatibility] =
    useState<ArCompatibilityReport | null>(null);

  useEffect(() => {
    setIsIosDevice(isLikelyIosClient());
    let cancelled = false;
    void getArCompatibilityReport().then((report) => {
    ...
    <main className="min-h-screen bg-[#f7f8f5] text-zinc-950">
      <div className="mx-auto max-w-3xl px-4 pt-[calc(3.5rem+env(safe-area-inset-top)+2rem)] pb-16 sm:px-6 md:pb-20">
        <div className="flex items-center justify-between gap-3">
          <Link
            href="/dashboard"
            className="text-sm text-zinc-600 transition-colors hover:text-emerald-950"
          >
            ← Dashboard
          </Link>
          <div className="hidden items-center gap-2 rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-[11px] font-medium text-zinc-600 shadow-sm sm:flex">
        ...
        <motion.div
          ...
          className="mt-6"
        >
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-[0_16px_45px_rgba(15,23,42,0.08)] sm:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-relaxed text-zinc-600">
                  Helix progressively enters WebXR, native mobile AR, camera preview, or interactive orbit fallback. It never claims a validation or AR mode that the browser has not exposed.
                </p>
              </div>
            </div>
            {compatibility ? (
              <div className="mt-4 rounded-xl border border-emerald-900/15 bg-emerald-50 px-3 py-2 text-xs leading-relaxed text-emerald-950">
                <span className="font-semibold">Detected AR path:</span>{" "}
                {compatibility.preferredMode.replaceAll("-", " ")}.{" "}
                {compatibility.blockers.length > 0
                  ? compatibility.blockers.join(" ")
                  : "No blocking compatibility issue detected."}
              </div>
            ) : null}

            {!isEntered ? (
              <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-[1fr_260px]">
                <div className="relative overflow-hidden rounded-2xl border border-zinc-200 bg-[#fbfcf9] p-5">
                  <div className="relative">
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={onEnter}
                        className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0b3d2e] px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-[#082d22]"
                      >
                        Enter AR preview
                      </button>
                      {qrSrc ? (
