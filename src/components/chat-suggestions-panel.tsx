...
export function ChatSuggestionsPanel({
  ...
  return (
    <div
      className={cn(
        "shrink-0 border-b border-zinc-800/90 bg-[#0b0a08] px-3 py-3.5 sm:px-4 sm:py-4",
        className,
      )}
    >
        ...
        >
          {suggestions.map((text) => (
            <button
              ...
              onClick={() => onPick(text)}
              className={cn(
                "max-w-full truncate rounded-full border border-amber-500/40 bg-amber-950/50 px-3 py-1.5 text-left text-[11px] leading-snug text-amber-100/95 transition-colors",
                "hover:border-amber-400/60 hover:bg-amber-900/55 hover:text-white",
                "disabled:pointer-events-none disabled:opacity-40",
              )}
            >
              {text}
